 ...
 * 容器是否包含另一个集合
 */
function datacontains(container, collection) {
  return !!collection && collection.every(function(item) {
      return datafindEntity(container, item.store_no) !== -1;
    });
}
 ...
 */
function datafindEntity(collection, id) {
...
angular.module('components.dataSelectorByShops')
  .controller('DataSelectorShopCtrl', ['$ccGrid', '$ccTips', 'dataShopsResources', '$scope', 'data', 'modalInstance', function($ccGrid, $ccTips, dataShopsResources, $scope, data, modalInstance) {
    ...
    // 表格指令配置
    this.gridOptions = {
      rowTpl: GLOBAL_STATIC.rootModule + '/components/dataSelectorByShops/tpls/row.tpl.html',
      headerTpl: GLOBAL_STATIC.rootModule + '/components/dataSelectorByShops/tpls/header.tpl.html',
    ...
    this.getSearchColumn = function(type) {
      dataShopsResources.searchColumn.get({type: type}, function(res) {
    ...
    this.getCloume = function(type) {
      dataShopsResources.cloume.get({type: type}, function(res) {
    ...
    this.refreshGrid = function() {
      this.gridOptions.resource = dataShopsResources.choose;
    ...
    this.switchSelectAll = function(allSelected, selectedCollection) {
      ...
      selectedCollection.forEach(function(entity) {
        var index = datafindEntity(self.items, entity.store_no);
    ...
    this.switchSelectItem = function($selected, id) {
      if ($selected) {
        ...
        this.items.splice(datafindEntity(this.items, id), 1);
    ...
    this.isEntitySelected = function(id) {
      return datafindEntity(this.items, id) !== -1;
    }
    ...
    Object.defineProperty(this, '$allSelected', {
      // 全选标识
      get: function() {
        return !!this.items.length && !!this.gridOptions.data && datacontains(this.items, this.gridOptions.data);
      }
    });
  }]);
