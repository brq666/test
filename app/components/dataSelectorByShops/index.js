// the samme as newSelectorByShops， 由于多次引入bug， 展示处理，后续抽取至组件库。
angular.module('components.dataSelectorByShops', [])
	.factory('dataSelectorShops', ['$ccModal', '$rootScope', function($ccModal, $rootScope) {
    return function(data) {
      return $ccModal.modal({
        title: '店铺选择器',
        scope: $rootScope.$new(),
        body: GLOBAL_STATIC.rootModule + '/components/dataSelectorByShops/tpls/body.html',
        footer: GLOBAL_STATIC.rootModule + '/components/dataSelectorByShops/tpls/footer.html',
        controller: 'DataSelectorShopCtrl',
        controllerAs: '$ctrl',
        locals: {
          data: data
        },
	...
	.directive('dataSelectorTreeManager', ['$compile', 'dataShopsResources', function($compile, dataShopsResources) {
      ...
      this._createTree = function(data, parentNode, createAll) {
        ...
        var html = data.map(function(item) {
            var html = angular.element([
                ...
                '<span ng-class="{basement: ' + (!!item.isFinal) + '}">' + item.name +'</span>',
                '</h4>',
                '</li>'
              ].join(''))[0];
