...
(function($) {
    $.addFlex = function(t, p) {
        ...
        var g = {
            ...
            dragMove: function(e) {
                if (this.colresize && p.colMoveAble) { //column resize
            ...
            addData: function(data) { //parse data
                ...
                //遍历data每个数据,转义文本 issue:CCMSSAAS-1977
                if (data.data && data.data.length) {
                    data.data.forEach(function(val, index) {
                        for (var key in val) {
                            if (typeof val[key] === 'string') {
                                val[key] = htmlEntities(val[key], false);
                            }
                        }
                    });
                }
                ...
                if( data[p.jsonStore.k_rp] ){
                  p.rp = data[p.jsonStore.k_rp];
                }
                ...
                var selectState = $('tr.trSelected', t).length ? true : false,
                    selectRow = [];
                if (selectState) {
            ...
            buildpager: function() { //rebuild pager based on new properties
                ...
                if(p.useRp) {
                    $('.pDiv2 select[name=rp] option').each(function() {
                        if($(this).val() == p.rp) {
                    ...
                    $('.pDiv2 select[name=rp]').each(function() {
                        $(this).val(p.rp);
                    });
                }
            },
            populate: function() { //get latest data
                ...
                if (p.updateDefaultParam) {
                    if (p.updateDefaultParam !== 'custom') {
                        param = [{
                            name: 'page',
                            value: p.newp
                        }, {
                        ...
                        var defaultMark = ['newp', 'rp', 'sortname', 'sortorder', 'query', 'qtype'];
                        p.customParam = p.customParams || [];
                        for (var i = 0; i < p.customParams.length; i++) {
                            param.push({
                                name: p.customParam[i],
                                value: p[defaultMark[i]]
                            })
                        }
                    }
                } else {
            ...
            setWidth: function(flag) { //宽度自适应 ps:当缩放窗口时宽度自适应还没实现
                var widthTotal = 0;
                ...
                return flag ? parseInt(gridWidth * cm.width / widthTotal) : parseInt(gridWidth * cm.width / widthTotal) - 20; //20为div的padding值
            },
        ...
        if (p.colModel) { //create model if any
            ...
            for (var i = 0; i < p.colModel.length; i++) {
                ...
                if (cm.name) {
                    $(th).attr({ 'abbr': cm.sortname || cm.name, 'dataindex': (cm.dataindex || '') });
                }
                if (cm.sortable) {
        ...
        if (p.usepager) {
            ...
            if (p.searchitems) {
                ...
                */
                ...
                //离开显示默认文字
                $('#flexigird_search').focusout(function() {
                    $('div.search_wrap label.default_color').css('display', 'block');
                });
        ...
        if (p.height == 'auto') { //重写自适应高度
            ...
            $(g.bDiv).height($(g.gDiv).parent().height() - getTotalH());
            ...
            $(window).bind('resize', function() {
                ...
                // 浏览器缩放
                if(p.colAutoWidth) {
                    $('thead tr:first th', g.hDiv).each(function(k) {
                        cm.width = p.autoWidthAry[k];
                        $('div', this).width(g.setWidth());
                        $('tbody tr',g.bDiv).css({overflow:"hidden", display:"inline-block"});
                        $(g.bDiv).css("overflow-x","hidden");
                        $('tbody tr',g.bDiv).each(function(key) {
                          $('tbody tr:eq('+ key +') td:eq('+ k +') div',g.bDiv).width(g.setWidth()).css({overflow:"hidden", display:"inline-block"});
                        });
                    })
                }
            });
