...
(function($){
	...
	_handler = {
		...
		onMousedownNode: function(eventMouseDown, _node) {
			...
			if (eventMouseDown.target.className== 'customerdeleteCreateNode' || eventMouseDown.button == 2 || !setting.edit.enable || (!setting.edit.drag.isCopy && !setting.edit.drag.isMove)) return true;
			...
			function _docMouseMove(event) {
				...
				if (root.dragFlag == 0) {
					...
					for (i=0, l=nodes.length; i<l; i++) {
						...
						tmpDom.css('width',$$(tmpNode, consts.id.A, setting).width());
	...
	_view = {
		...
		editNode: function(setting, node) {
			...
			/* ztree Դ���޸ģ�����node����ʾɾ���� -PEACH
			* <em class='customerdeleteCreateNode'>X</em>"
			*  view:���ò��� cutomDeleteElement��true || false���Ƿ���ʾɾ����
			*/
			if(setting.view.cutomDeleteElement && !node.id){
				$$(node, consts.id.SPAN, setting).html("<input type=text class='rename' id='" + node.tId + consts.id.INPUT + "' treeNode" + consts.id.INPUT + " ><em class='customerdeleteCreateNode'>X</em>");
			}else{
			...
			inputObj.siblings("em").bind("click",function(){
				//$$(node, setting).remove();
				view.removeNode(setting, node);
				return false
			});
