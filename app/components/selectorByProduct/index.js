angular.module('components.selectorByProduct', [])
  .factory('selectorProduct', ['$ccModal', '$rootScope', function ($ccModal, $rootScope) {
    var modalOptions = {
      scope: $rootScope.$new(),
      body: GLOBAL_STATIC.rootModule + '/components/selectorByProduct/tpls/body.html',
      footer: GLOBAL_STATIC.rootModule + '/components/selectorByProduct/tpls/footer.html',
      controller: 'selectorProductCtrl',
      controllerAs: '$ctrl',
      style: {
        'width': '1000px',
        'min-width': '1000px',
  ...
  .directive('selectorTreeManagerByProduct', ['$compile', 'ShopsResources', function ($compile, ShopsResources) {
    return function (scope, element, attrs) {
      ...
      this._bindLoadNextTreeEvent = function () {
        ...
        treeListEle.addEventListener('click', function (evt) {
          ...
          const selected = evt.target.tagName !== 'I';
          const pkid = target.getAttribute('pkid');
          const level = target.getAttribute('level');

          // 行为是否要选中当前项
          if (selected) {
            
            that.ctrl.tree = that.ctrl.bindings.platCode === 'edecathlon' ? { level: level } : { areaId: pkid, level: level };
            that.ctrl.query();
          ...
          if (target.getAttribute('created')) {
            ...
            ShopsResources.tree.get({
              pkid: pkid
            }, function (res) {
              that._createTree(res.list || [], target.parentNode, true);
      ...
      this._loadRootTree = function () {
        ...
        var platCode = that.ctrl.bindings.platCode;
        if (platCode === 'edecathlon') {
          ShopsResources.edecathlonTree.get({
            pkid: '000'
          }, function (res) {
            var parent = that.$element[0];
            var liElement = parent.querySelector('.menu .item > li');
            var rootNode = angular.element('<h4 class="root-tree" pkid="' + res.list.productOrgCode + '" level="' + res.list.treeCode + '">'
              + '<i class="icon-arrow root-tree-toggle"></i>'
              + '<span>' + res.list.productOrgName + '</span>'
              + '</h4>');
            angular.element(liElement).append(rootNode);
            // 创建第一层
            that._createTree(
              res.list.children || [],
              liElement,
          ...
          ShopsResources.tree.get({
            ...
            that._createTree(
              res.list || [],
              parent.querySelector('.menu .item > li'),
              true
            );
            // 设置打开状态
            that._setOpen(parent.querySelector('.root-tree'));
      ...
      this._createTree = function (data, parentNode, createAll) {
        ...
        if (platCode === 'edecathlon') {
          html = data.map(function (item) {
            var html = angular.element('<li text="' + item.productOrgName + '">'
              + '<h4 pkid="' + item.productOrgCode + '" level="' + item.treeCode + '"' + (createAll ? 'created="true"' : '') + '>'
              + '<i ng-if="' + (!item.final) + '" class="icon-arrow"></i>'
              + '<span ng-class="{basement: ' + (!!item.final) + '}" title="' + item.productOrgName + '">' + item.productOrgName + '</span>'
              + '</h4>'
              + '</li>')[0];

            $compile(html)(that.$scope);

            if (!that.treeVoObj[item.productOrgCode]) {
              that.treeVoObj[item.productOrgCode] = item.children || [];
            }

            if (createAll && that.treeVoObj[item.productOrgCode].length) {
              that._createTree(item.children, html, createAll);
            }

            if (item.final || (item.children && !item.children.length)) {
              //异步请求，去除代码
          ...
          html = data.map(function (item) {
            var html = angular.element('<li text="' + item.name + '">'
