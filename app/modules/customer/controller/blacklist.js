$('.blacklistMobile').flexigrid({
  url: rootStatic + 'channel/blacklist/MOBILE/page/',
  method: 'GET',
  dataType: 'json',
  colModel: [{
    display: '手机',
...
var mobile = {
  ...
  loadBlacklist: function(obj) {
    ...
    if (str == "") {
      ...
      if (this.validationPhone(str)) {
        $.ajax({
          url: rootStatic + 'channel/blacklist/MOBILE/exists?value=' + str,
          async: false,
          type: "GET",
          cache: false,
          dataType: "json",
  ...
  createBlacklist: function() {
    ...
    if (flag1 && flag2) {
      $.ajax({
        url: rootStatic + 'channel/blacklist/MOBILE',
        async: false,
        type: "POST",
        data: parame,
        dataType: "json",
  ...
  validationPhone: function(str) {
    var partten = /^0{0,1}(13[0-9]|14[0-9]|15[0-9]|17[0-9]|18[0-9])[0-9]{8}$/;
    if (partten.test(str)) {
      return true;
    } else {
      return false;
