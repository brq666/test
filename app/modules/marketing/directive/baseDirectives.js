(function() {
  var directives = angular.module('marketingCommondirectives', []);
  //角色管理左边的tree
  ...
  directives.directive("outRootTree", function() {
    ...
    function linkFn(scope, elem, attrs) {
      ...
      scope.nodeClick = function(event, treeId, treeNode, flag) { // flag 判断是哪个ztree
        ...
        var marketActivityPageDom = angular.element('.marketActivity-page');
        ...
        if (flag == "activity") { //活动选择器
          ...
          // 同marketActivityPage页面controller 交互
          if (marketActivityPageDom.length > 0) {
            marketActivityPageDom.scope().onTreeChange(treeNode);
          }
        };
      };
      scope.isCanDrap = function(treeId, treeNodes) {
    ...
    return {
      template: '<div class="market_tree_menu grey_border" id="tree_menu">\
                    <div id="tree_node_add" class="market_tree_menu_item" ng-click="addNode()">增加{{menuType}}</div>\
                     <!-- <div  id="tree_node_remove" class="market_tree_menu_item tree_menu move_item" ng-click="removeNode()">移动{{menuType}}</div> -->\
                    <div  id="tree_node_remove" class="market_tree_menu_item" ng-click="removeNode()">删除{{menuType}}</div>\
                    <div  id="tree_node_rename" class="market_tree_menu_item" ng-click="renameNode()">重命名</div>\
                  </div>',
      link: linkFn,
      replace: true
    };
    ...
    function linkFn(scope, elem, attrs) {
      ...
      function Menu() {
        ...
        this.show = function(type, x, y, e) {
          ...
          el.css({
            "top": maxMenuHeight > $(window).height() ? (y - ztreeMenuScrollTop) + "px" : (y - ztreeMenuScrollTop - 40) + "px",
        ...
        function onBodyMouseDown(event) {
          if (!(event.target.id == "tree_menu"
            || $(event.target).parents("#tree_menu").length > 0)
  ...
  directives.directive('inputActivityName', ['$parse', function($parse) {
    return function(scope, elem, attrs) {
      ...
      var maxlength = attrs.maxlength * 1 || 20;
      elem.on(navigator.userAgent.indexOf("MSIE 8.0") > 0 ? "propertychange" : 'input', function(event) {
        var val = event.target.value;
        if (!val) {
          return;
      ...
      function checkName(event) {
        var val = event.target.value;
        //如果从复制过来的不合规范的内容
        if (nReg.test(val) || val.length > maxlength) {
          val = val.replace(nReg, "");
          event.target.value = val;
        }
        if (attrs.inputActivityName != 'true' && val.length > maxlength) {
