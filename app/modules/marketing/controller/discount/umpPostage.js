angular.module("campaign.controllers").controller('ccmsUmpPostage', ['$scope', '$location', '$http', '$compile', 'getListService', '$q', '$rootScope',
  function($scope, $location, $http, $compile, getListService, $q, $rootScope) {
    ...
    $scope.tpl = {
      ...
      "add": function() {
        ...
        getListService.getCityByType(function(response) {
          $scope.cityIds = [];
          $scope.cityLists = [];
          $.ajax({
            async: false,
            type: "GET",
            "url": GLOBAL_STATIC.componentRoot + "area/selector/0/0/",
            success: function(responseB) {
              responseB.forEach(function(item) {
                $scope.cityLists.push({
                  "id": item.id,
                  "name": item.name
                });
              })
          ...
          $.ajax({
            ...
            success: function(responseA) {
              ...
              for (var i = 0; i < responseA.length; i++) {
                ...
                cityIds = cityIds.concat(ids) + ",";
              }
              cityIds = cityIds.substring(0, cityIds.length - 1);
              $scope.tpl.postagveArea = cityIds.replace(/,/g, "*");
              $scope.cityTip = $scope.cityLists.length > 0 ? "选择了35个省": "";
        ...
        this.src = CAMPAIGN_STATIC.tplBasePath + "view/discount/addUmpPostage.html?_=" + ( + new Date);
      },
      "modify": function(id, curShopName, status) {
        ...
        $.ajax({
          ...
          success: function(res) {
            ...
            _this.postagve = res.postagve;
            _this.postagveArea = res.postagveArea; //包邮城市 **
            if (_this.postagveArea != null && _this.postagveArea != "" && _this.postagveArea != undefined) {
            ...
            _this.snapshootId = $scope.snapshootId = res.snapshootId;
      ...
      "save": function(moreTime) {
        ...
        parame.postagve = _this.postagve;
        ...
        parame.type = 2; //0 商品级  1包邮卡 2 包邮卡
        ...
        $.ajax({
          ...
          success: function(res) {
            ...
            if (res.code == "success") {
              ...
              removeAlert();
              $('.ump_postage_flexgrid').flexReload();
              _this.src = "";
              ...
              _this.src = CAMPAIGN_STATIC.tplBasePath + "view/discount/addUmpPostage.html";
    ...
    $scope.$watch("tpl.userScopeType", function(n, o) { //监听优惠范围
      ...
      if (n != undefined) {
        ...
        if ($scope.shopForCounpon && $scope.shopForCounpon.id) {
          $scope.tpl.shopCounts($scope.shopForCounpon.id, $scope.tpl.userScopeType)
        }
      }
    });
    ...
    $scope.list = {
      "searchButtonClick": function(hdVal) {
        var _this = angular.element(".ump_postage_flexgrid")[0];
        _this.p.newp = 1;
        _this.p.qtype = "name";
        _this.p.query = hdVal || "";
        _this.grid.populate();
    ...
    $scope.empower = {
      ...
      "init": function(id, item) {
        ...
        if ($scope.item == "listToApply") {
          ...
          $scope.empower.applyContent = myTime + "创建'" + $scope.shopForCounpon.name + "'包邮卡优惠'" + $scope.tpl.name + "',请授权。退订回复TD";
          $(".empowerApply").addInteractivePop({
            magTitle: "授权申请",
            mark: false,
            drag: true,
          ...
          $scope.tpl.src = CAMPAIGN_STATIC.tplBasePath + "view/discount/addUmpPostage.html";
        }
    ...
    $('.ump_postage_flexgrid').flexigrid({
      url: GLOBAL_STATIC.campaignRoot + 'ump/postage/',
      method: 'GET',
      params: [{
        "name": "status",
        "value": "7"
      ...
      { display: '优惠商品范围', name: 'postagve', width: 2, sortable: false, align: 'left', mapping: ["goodsScopeType"], convert: function (v, mappval) {
        if (mappval[0] == 0) {
          return "全部商品优惠";
        } else if (mappval[0] == 1) {
          return "选择享受优惠商品";
