(function (window, angular, undefined) {
    ...
    app.controller('dclonsmsCtrl', ["$scope", "$http", "$compile", "$window", "$location", "getListService", "dclonService", "$sce", "$q", "$rootScope",
        function ($scope, $http, $compile, $window, $location, getListService, dclonService, $sce, $q, $rootScope) {
            ...
            $scope.getGoodsOfShop = function (param) {
                ...
                dclonService.sms.getGoodsOfShop(params, callback);
            }
            //选择短链商品
            ...
            $scope.getShortLinkType = function () {
                ...
                dclonService.sms.getShortLinkType(callback);
            }
            ...
            //获取H5链接店铺
            var defaultShopId = window.CCMS_INFOS_CONFIG.H5MODELShop || 'belle_1001';
            ...
            $scope.getMessageOfHFive = function (param) {
                ...
                dclonService.sms.getMessageOfHFive(params, callback);
            }
            ...
            $scope.insertHFiveChain = function (e) {
                ...
                if ($scope.checkedHFiveUrl === true) {
                    $scope.checkedHFiveUrl = false;
                    $scope.editorHide = true;
                    var name = $scope.HFiveTitle + '(35个字)';
                    //var id = $scope.HFiveUrl;
                    var id = 'h5^' + $scope.pltType + '^' + $scope.tenant + '^' + 0 + '^' + $scope.HFivePreview;
                    var mozilla = 'mozilla';
                    var shopid = $scope.shopId;
            ...
            $scope.getCardList = function () {
                ...
                dclonService.sms.getCardList(callback);
            }
            ...
            $scope.insertCustomChain = function (e) {
                ...
                dclonService.sms.getShortLink(params, callback);
            }
            ...
            $scope.getSmsMessage = function () {
                var callback = function (dat) {
                    ...
                    $scope.data.signature = dat.signature || "迪卡侬";
                    ...
                    if ($scope.data.blacklist.customer.length === 0 && !$scope.data.content) {
                        $scope.blacklistTip = true;
                    }
                    else {
                        var msg = "";
                        ...
                        if ($scope.data.blacklist.customer.length > 0) {
                            $scope.blacklistTip = $scope.data.blacklist.customer[0].name == 1 ? true : false;
                        } else {
                            $scope.blacklistTip = true;
                        }
                    }
                    ...
                    if ($scope.data.redlist.length > 0) {
                        $scope.redlistTip = $scope.data.redlist[0].name == 1 ? true : false;
                    } else {
                        $scope.redlistTip = true;
                    }
                ...
                dclonService.sms.getSmsMessage(
                    callback, //callback
                    $scope.model //表单数据
                );
            ...
            $scope.saveSmsMessageCheck = function (e) {
                ...
                dclonService.sms.getErrorWords(
                    callback, //callback
                    $scope.model, //表单数据
                    callback
                )
                    ...
                    if (regValue.test(data[i])) {
                        if ($(data[i])[0].name) {
                            if (jQuery.browser.mozilla) {
                                ...
                                obj.id = 'h5^' + $(data[i])[0].name + '^' + $(data[i])[0].align + '^' + 0 + '^' + $(data[i])[0].src;
                            }
                            else {
            ...
            $scope.ConfirmMessagesCounts = function () {
                ...
                $scope.sentData.redlist = [{id: 1001, name: $scope.redlistTip === true ? 1 : 0}];
                $scope.sentData.blacklist = {customer:[{id: 1001, name: $scope.blacklistTip === true ? 1 : 0}]};
                ...
                if ($scope.numCheck && !$scope.ConfirmMessagesCounts.ajaxFlag) {
                    ...
                    dclonService.sms.SaveSmsMessage(
                        callback, //callback
                        $scope.sentData, //表单数据
                        errorCallback
                    )
            ...
            $scope.gridObj = {
                ...
                "itemdown": function (jobId) {
                    dclonService.sms.getSmsDownLoad(function (response) { }, graph.campId, graph.nodeId, jobId)
                }
            }
            ...
            $scope.getReports = function () {
                ...
                dclonService.sms.senderSmsSummary(function (response) {
                ...
                $('.dataInfoList').flexigrid({
                    url: GLOBAL_STATIC.nodeRoot + 'report/sender/dclonsms/list?campId=' + graph.campId + '&nodeId=' + graph.nodeId + '&jobId=',
                    method: 'GET',
                    dataType: 'json',
                    headers: {
...

                        {
                            ...
                            convert: function (v, mappVal) {
                                if (!mappVal[1]) {
                                    ...
                                    var href = '/node/v1/web/report/sender/dclonsms/download/?campaignId=' + graph.campId + '&jobId=' + mappVal[0] + '&nodeId=' + graph.nodeId;
    ...
    app.controller('dclonsignCtrl', ["$scope", "$http",
        function ($scope, $http) {
            ...
            $scope.addItem = function () {
                ...
                $http.post(GLOBAL_STATIC.nodeRoot + "node/dclonsms/signature/", {
                    name: $scope.itemName
            ...
            $scope.deleteItem = function (item, $event) {
                ...
                $(this).Confirm({
                    ...
                    $http({
                        method: 'DELETE',
                        url: GLOBAL_STATIC.nodeRoot + "node/dclonsms/signature/" + item.id
            ...
            $scope.getAllItem = function () {
                $http.get(GLOBAL_STATIC.nodeRoot + "node/dclonsms/signature/" + "?_=" + new Date().getTime()).success(getDataSucc).error(getDataErr);
                //成功获取节点信息
