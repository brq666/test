function discountISNewCtrl($scope, $rootScope, $http, getListService, $compile, $q) {
  ...
  $scope.isScopeObj = {
    ...
    "showIsDataPop": function() {
      ...
      $('.dataInfoList').flexigrid({
        url: GLOBAL_STATIC.nodeRoot + 'node/integralsendnew/report/?nodeId=' + graph.nodeId + "&_=" + new Date().getTime(),
        method: 'GET',
        dataType: 'json',
        colModel: [{
          display: '客户提交时间',
          ...
          convert: function(v, mappVal) {
            var href = '{{appOrigin}}/node/integralsend/download/?fileName=' + v;
    ...
    "fillData": function() {
      var _this = this;
      getListService.openDiscountIsNew(function(response) {
        _this.name = response.name || "优惠券";
    ...
    "postDiscountIsNew": function(ent) {
      var _this = this;
      ...
      disposeCommMethod.shuyunAjaxButtonClickMethod(function() {
        getListService.postDiscountIsNew(function(response) {
      ...
      if (data) {
        ...
        for (var i = 0; i < len; i++) {
          if (eleName == "useHdInput") {
            ...
            $ul.append('<li style="margin:0;"><a href="javascript:void(0);" id=' + data[i].ctyId + '>' + data[i].ctyName + '</a></li>');
            $ul.find("a").css({
              "padding": "3px 10px",
              "color": "#3D3D3D",
              "display": "block"
