...
angular.module("dataManage.controllers").controller('phoneBlackListCtrl', ["$scope", "$window", "$location", "$http", "$compile", "ngRedAndBlackCustomService", "FileUploader",
    function($scope, $window, $location, $http, $compile, ngRedAndBlackCustomService, FileUploader) {
        ...
        $scope.showEditBulkPop = function() {
            $scope.select_Group = true //默认持原有分组不变选中
            $scope.edittype = "list";
            $scope.intiEidtPop();
            $scope.getSelectedIds();
            ...
            if (!$scope.editRedCustom.groupName && !$scope.select_Group) {
                ...
                if ($scope.edittype == "list") {
                    ...
                    if ($scope.selectMode == "BY_QUERY" && $scope.checkStatus) //全选状态
                    {
                        ...
                        editCustomList.queryChecklistType = "MOBILE";
                        editCustomList.selectMode = $scope.selectMode;
                        //editCustomList.queryPlatCode = $scope.queryPlatCode;
        ...
        $scope.deleteRedBulk = function() {
            ...
            if ($scope.selectedIds.length == 0) {
                ...
                if ($scope.selectMode == "BY_QUERY" && $scope.checkStatus) //全选状态
                {
                    deleteCustomList.queryChecklistType = "MOBILE";
                    deleteCustomList.selectMode = "BY_QUERY";
                    //deleteCustomList.queryPlatCode = $scope.queryPlatCode;
        ...
        $scope.reset = function() {
                ...
                $scope.redGroupValue = "";
                $scope.platStatusText = "请选择平台";
                $scope.platStatusValue = "";
                $scope.hdValue = "";
                ...
                //curGridElement.grid.removeParams("platCode");
                angular.element("#couponListsGrid").flexReload();
                $scope.checkStatus = false;
            }
            //导出
        ...
        $scope.redListSave = function() {
            ...
            //批量数据上传
                ...
                var redCustmerData = {};
                redCustmerData.checklistType = "MOBILE";
                //redCustmerData.platCode = "";
        ...
        $('#couponListsGrid').flexigrid({
            url: GLOBAL_STATIC.componentRoot + 'checklist/?checklistType=MOBILE',
            method: 'GET',
            dataType: 'json',
            headers: {
            ...
            colModel: [{
                ...
                convert: function(v, mappVal) {
                    return '<a ng-click="gridObj.redListEidt(\'' + mappVal[0] + '\',\'' + mappVal[1] + '\')"  href="javascript:void(0);" class="edit_delete edit_icon" title="修改" ></a>' + '<a href="javascript:void(0);" class="edit_delete delete_icon" title="删除此手机信息"  ng-data-red-delete-attr ></a>'
                }
            }],
            sortname: 'lastUpdate',
            ...
            onSuccess: function(data) {
                ...
                gridIsCompile = true;
                var le = ( $('.nosortable').eq(0).width() - 84 ) /2;
                $('.blackphonehide').css({
                  'left': le
                })
                $('.blackphonehide').show()
        ...
        ngRedAndBlackCustomService.checkDownLoad('MOBILE', function () {
          console.log('MOBILE有权限');
          $scope.downLoadPermissions = true;
        }, function () {
          console.log('MOBILE无权限');
