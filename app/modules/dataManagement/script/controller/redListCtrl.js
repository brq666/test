 ...
 */
angular.module("dataManage.controllers").controller('redListCtrl', ["$scope", "$window", "$location", "$http", "$compile", "ngRedAndBlackCustomService", "FileUploader",
  function($scope, $window, $location, $http, $compile, ngRedAndBlackCustomService, FileUploader) {
      ...
      if (!$scope.editRedCustom.groupName && !$scope.select_Group) {
        ...
        if ($scope.edittype == "list") {
          ...
          if ($scope.selectMode == "BY_QUERY" && $scope.checkStatus) {
            ...
            editCustomList.queryChecklistType = "WHITE";
    ...
    $scope.deleteRedBulk = function() {
      ...
      if ($scope.selectedIds.length == 0) {
        ...
        {
          deleteCustomList.queryChecklistType = "WHITE";
    ...
    $scope.redListSave = function() {
      ...
      if ($scope.status == 0) {
        ...
        var redCustmerData = {};
        redCustmerData.checklistType = "WHITE";
    ...
    /* labelSelected 为选中为state_executed状态的标签做标记 */
    ...
    $('#couponListsGrid').flexigrid({
      url: GLOBAL_STATIC.componentRoot + 'checklist/?checklistType=WHITE',
      method: 'GET',
      dataType: 'json',
      headers: {
