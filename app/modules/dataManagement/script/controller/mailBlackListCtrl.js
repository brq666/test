 ...
 */
angular.module("dataManage.controllers").controller('mailBlackListCtrl', ["$scope", "$window", "$location", "$http", "$compile", "ngRedAndBlackCustomService", "FileUploader",
...
//关闭弹框分组
    ...
    $scope.showEditBulkPop = function() {
      $scope.select_Group = true //默认持原有分组不变不选中
      $scope.edittype = "list";
      $scope.intiEidtPop();
      $scope.getSelectedIds();
      ...
      if (!$scope.editRedCustom.groupName && !$scope.select_Group) {
        ...
        if ($scope.edittype == "list") {
          ...
          if ($scope.selectMode == "BY_QUERY" && $scope.checkStatus) //全选状态
          {
            ...
            editCustomList.queryChecklistType = "EMAIL";
    ...
    $scope.deleteRedBulk = function() {
      ...
      if ($scope.selectedIds.length == 0) {
        ...
        if ($scope.selectMode == "BY_QUERY" && $scope.checkStatus) //全选状态
        {
          deleteCustomList.queryChecklistType = "EMAIL";
    ...
    $('#couponListsGrid').flexigrid({
      url: GLOBAL_STATIC.componentRoot + 'checklist/?checklistType=EMAIL&tenantId=' + DATA_STATIC.tenantId,
      method: 'GET',
      dataType: 'json',
      headers: {
      ...
      colModel: [{
        display: '<span ng-class={cloneCheckbox:true,cloneCheckboxChecked:checkStatus} ng-click="gridObj.selectChange()"  ></span>' + '<select class="commSelect width80"  ng-model="selectMode"  ng-change="gridObj.changeCheckbox()" ' + ' ng-options="dataCover.selectMode as dataCover.selectText for dataCover in gridObj.dataCoverList"></select>',
        sortable: false,
        name: 'enable',
        width: 1,
        dataindex: 'enabled',
      ...
      },
      {
        ...
        convert: function(v, mappVal) {
          return '<a ng-click="gridObj.redListEidt(\'' + mappVal[0] + '\',\'' + mappVal[1] + '\')"  href="javascript:void(0);" class="edit_delete edit_icon" title="修改" ></a>' + '<a href="javascript:void(0);" class="edit_delete delete_icon"  title="删除此邮箱信息"  ng-data-red-delete-attr ></a>'
        }
      }],
      sortname: 'lastUpdate',
    ...
    ngRedAndBlackCustomService.checkDownLoad('EMAIL', function () {
      console.log('EMAIL有权限');
      $scope.downLoadPermissions = true;
    }, function () {
      console.log('EMAIL无权限');
