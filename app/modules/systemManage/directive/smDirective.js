    ...
    function Menu() {
      ...
      this.show = function(type, x, y, e, node) {
        ...
        if(attrs.pos=="changeTop"){
          el.css({
            "top": maxMenuHeight > $(window).height() ? (y - ztreeMenuScrollTop - 60)-30 + "px": (y - ztreeMenuScrollTop)-30 + "px",
            "left": (x - ztreeMenuScrollLeft) + "px",
            "dispaly": "block"
          });
        }else{
...
directives.directive('partitionZtree', function() {
  ...
  function linkFn(scope, elem, attrs) {
    ...
    function onRename(e, treeId, treeNode, isCancel) {
        //新增加的节点
        if (!treeNode.id) {
        _checkNameExist(treeNode.categoryName);
          //上传父节点的id
          treeNode.groupCnt=0;
...
directives.directive('customerNodeZtree', function() {
  ...
  function linkFn(scope, elem, attrs) {
    ...
    function onClick(event, treeId, treeNode) {
      scope.nodeClick(event, treeId, treeNode, "activityNode");
    }
...
directives.directive('commSelectConfig', function() {
  return function(scope, element, attrs) {
    ...
    var arrowsHeight = top + (eleHeihgt - 19) / 2;
     if (attrs["class"].indexOf("par") != -1) { // 定位下拉箭头位子
          arrowsWidth =2*eleWidth-53;
          arrowsHeight=2*eleHeihgt+11
        }
    $a.css({
      "left": arrowsWidth,
      "top": arrowsHeight
